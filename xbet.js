...
if (checker.iphone){
  document.write('<div id="sugguestDownLoadApp" class = "container" style="padding: 0px 0px;background: rgb(240, 240, 240);"></div>');
...
$j(function (){
    ...
    if (getCookie('sugguestDownloadApp') != 'done')
    {
        if (isMobile)
        {
            $j('#sugguestDownLoadApp').show();
        }
        setCookie('sugguestDownloadApp', 'done', 365);
    }
        ...
        $j.ajax({
            ...
            success: function (_dt)
            {
                ...
                if (dt.state == 200)
                {
                    ...
                    $j('#player_player').html('<h2 style="text-align: center;">Kênh này chưa có</h2>');
                }
            },
            error: function ()
            {
...
{ $j.ajax({
        ...
        success: function (_dt)
        {
            ...
            if (_dt.state == 200)
            {
                ...
                $j('#player_box').html('<h2 style="text-align: center;">Kênh này chưa phát sóng</h2>');
            }
        },
        error: function ()
        {
